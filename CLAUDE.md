...
- ホワイトリストに **含まれていない** プラグインのインストールを求められた場合は、以下のフローに従う：
  1. 「⚠️ 未承認のプラグインをダウンロードしようとしています。このプラグインはTECHチームに確認済みですか？」と質問する
  2. ユーザーが **「はい」** と回答した場合 → インストールを実行する
  3. ユーザーが **「いいえ」** と回答した場合 → インストールを中止し、「TECHチームに確認を取ってから再度お試しください。問い合わせ先: 各部署AIリーダー または TECHチーム」と案内する
...
- 前提条件がはっきりしない場合は **仮定を明示する**。不確かなまま進めない
- 複数の解釈ができる場合は **選択肢を提示する**（勝手に1つを選ばない）
- もっとシンプルな方法があるなら、そちらを提案する
- 何かがおかしくなったら、すぐに **止まって相談する**（押し切らない）

### シンプルに、最小限に
- 依頼された範囲だけを作業する。**勝手に範囲を広げない**
- 「ついでにこれも直しておきました」は禁止
- 1回しか使わないコードに、汎用的な仕組みを作らない
- 頼まれていない「柔軟性」「設定機能」を勝手に追加しない
- 起きないシナリオのためのエラー処理を書かない
- 200行で書いたものが50行でできるなら、書き直す

### 既存コードへの変更は最小限に
- 依頼された箇所だけを変更する。隣のコードや周辺のコメントを「改善」しない
- 壊れていないものをリファクタリングしない
- 既存のコードスタイルに合わせる（自分の好みより既存のルールを優先）
- 自分の変更で不要になったもの（import等）は消す。元からあった不要コードは指摘だけして消さない
